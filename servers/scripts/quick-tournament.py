...
def printTeamRecords2(recordName, className, topFinishes):
  ...
  if len(topFinishes) > 0:
    ...
    for f in topFinishes:
      string += u'  <tr title="%s, %s"><td class="rank">%d.</td><td>%s</td><td>%s</td></tr>\n' % (escape(formatTimeExact(f[2])), escape(formatDate(f[3])), f[0], escape(formatDate(f[3])), f[1])
    string += '</table>\n'
  string += '</div>\n'
...
menuText += '</ul>'
print header("Quick Tournament #20 - DDraceNetwork", menuText, "")
...
print '<div id="global" class="block div-tournament"><h2>Quick Tournament #20</h2>'
print '<p>This tournament was played on 2014-09-28 at 20:00 CEST.</p>'
print '</div>'
