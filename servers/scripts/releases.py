...
for curType in types:
  ...
  for x in releases:
    ...
    slugifiedMapName = slugify2('%s' % originalMapName)
  ...
  for i, mapsString in enumerate(mapsStrings):
    ...
    with open(tmpname, 'w', encoding='utf-8') as tf:
      if curType:
        print(header("%s Server Map Releases (%d/%d) - DDraceNetwork" % (curType, i+1, len(mapsStrings)), menuText, ""), file=tf)
      else:
        print(header("Map Releases (%d/%d) - DDraceNetwork" % (i+1, len(mapsStrings)), menuText, ""), file=tf)
      print('<div id="global" class="block">', file=tf)
      print('<div class="right"><form id="mapform" action="/maps/" method="get"><input name="map" class="typeahead" type="text" placeholder="Map search"><input type="submit" value="Map search" style="position: absolute; left: -9999px"></form><br><form id="mapperform" action="/maps/" method="get"><input name="mapper" class="typeahead" type="text" placeholder="Mapper search"><input type="submit" value="Mapper search" style="position: absolute; left: -9999px"></form></div>', file=tf)
      if curType:
        print('<h2>%s Server Map Releases (%d/%d)</h2><br/>' % (curType, i+1, len(mapsStrings)), file=tf)
      else:
      ...
      if not curType:
        print('<a href="/releases/feed/"><img width="36" src="/feed.svg"/></a> You can subscribe to the feed to get updated about new map releases.<br>', file=tf)
      if curType:
        print('<a href="/releases/%s/maps.json"><img width="36" src="/json.svg"/></a> %s map release infos are also available in JSON format.' % (curType.lower(), curType), file=tf)
      else:
        print('<a href="/releases/maps.json"><img width="36" src="/json.svg"/></a> All map release infos are also available in JSON format.', file=tf)
      print('<p>Planned Map Releases are listed on <a href="https://discordapp.com/invite/85Vavs">Discord</a>. All DDNet maps can be download from <a href="https://github.com/ddnet/ddnet-maps">GitHub</a>, <a href="https://maps.ddnet.org/compilations/">our compilations</a> or <a href="https://maps.ddnet.org/">as single files</a>.</p>', file=tf)
      ...
      if len(mapsStrings) > 1:
        if curType:
          baseLink = '/releases/%s/' % curType.lower()
        else:
          baseLink = '/releases/'
