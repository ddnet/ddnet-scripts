...
class CustomResponse:
    ...
    def __iter__(self):
        # Make the object iterable by yielding bytes
        yield self.out.getvalue().encode("utf-8")  # Encode output to bytes

    def close(self):
        pass # This hangs the page unfortunately
        #global queries
        #print("Running expensive SQL queries...")
        #for query in queries:
        #  timestamp = r.get(f"ts:{query}")
        #  if not timestamp or timestamp < time.time() - 60:
        #    print(f"Running {query}")
        #    cur.execute(query)
        #    result = cur.fetchall()
        #    r.setex(f"sql:{query}", 86400, json.dumps(serialize(result)).encode("utf-8"))
        #queries = []
...
def serialize(result):
    if isinstance(result, list):
        ...
        return tuple(serialize(item) for item in result)
...
def deserialize(result):
    if isinstance(result, list):
        return [deserialize(item) for item in result]
    elif isinstance(result, tuple):
        return tuple(deserialize(item) for item in result)
    elif isinstance(result, dict):
        return {key: deserialize(value) for key, value in result.items()}
    try:
...
def getTeamRanks(originalMapName, country, mbCountry2, html):
  ...
  if country == None:
    rows = query("select distinct r.Name, r.ID, r.Time, r.Timestamp, (select substring(Server, 1, 3) from record_race where Map = r.Map and Name = r.Name and Time = r.Time limit 1) as Server, r.GameID from ((select distinct ID from record_teamrace where Map = '%s' ORDER BY Time limit 20) as l) left join record_teamrace as r on l.ID = r.ID order by r.Time, r.ID, r.Name;" % (con.escape_string(originalMapName).decode("utf-8")))
  else:
    rows = query("select distinct r.Name, r.ID, r.Time, r.Timestamp, SUBSTRING(n.Server, 1, 3), r.GameID from ((select distinct ID from record_teamrace where Map = '%s' ORDER BY Time) as l) left join record_teamrace as r on l.ID = r.ID inner join ((select * from record_race %s) as n) on r.Map = n.Map and r.Name = n.Name and r.Time = n.Time order by r.Time, r.ID, r.Name limit 160;" % (con.escape_string(originalMapName).decode("utf-8"), mbCountry2))
  if len(rows) > 0:
    ID = rows[0][1]
  ...
  for row in rows:
    if row[1] != ID:
      if currentPosition <= 20:
        ...
        teamRanks.append((currentRank, joinNames(fNames) if html else names, time, timestamp, foundCountry, foundGameId))
        names = []
...
def getFinishes(originalMapName, country, mbCountry):
  rows = query("select Name, count(*), sum(Time), min(Timestamp), max(Timestamp) from record_race where Map = '%s' %s group by Name order by count(*) desc limit 20;" % (con.escape_string(originalMapName).decode("utf-8"), mbCountry))
  ranks = []
...
def query(sql):
  global con, cur, r, queries
  cache_key = f"sql:{sql}"
  cached_result = r.get(cache_key)
  if cached_result:
      #queries.append(sql)
      return deserialize(json.loads(cached_result.decode("utf-8")))
  ...
  cache_time = int(duration * 60)
  if cache_time > 0:
      r.setex(cache_key, cache_time, json.dumps(serialize(result)).encode("utf-8"))
  #r.set(f"ts:{sql}", time.time())
  return result
...
def application(env, start_response):
  ...
  if path in ['/maps/', '/maps2/']:
    ...
    if 'json' in qs:
      ...
      rows = query(f"select Server, Points, Stars, Mapper, Timestamp from record_maps where Map = '{con.escape_string(originalMapName).decode('utf-8')}';")
      if len(rows) > 0:
        ...
        if country == "OLD": # Old ranks had no country
          ...
          mbCountry = "and Server like \"%s%%%%\"" % con.escape_string(country).decode("utf-8")
          mbCountry2 = "where Server like \"%s%%%%\"" % con.escape_string(country).decode("utf-8")
        else:
          mbCountry = ""
          mbCountry2 = ""
        ...
        if countFinishes:
          rows = query("select (select median(Time) over (partition by Map) from record_race where Map = '%s' %s limit 1), min(Timestamp), max(Timestamp), count(*), count(distinct Name) from record_race where Map = '%s' %s;" % (con.escape_string(originalMapName).decode("utf-8"), mbCountry, con.escape_string(originalMapName).decode("utf-8"), mbCountry))
        ...
        if type == "Solo" or type == "Race" or type == "Dummy":
          ...
          if country == None:
            rows = query("select count(Name) from record_teamrace where Map = '%s' group by ID order by count(Name) desc limit 1;" % con.escape_string(originalMapName).decode("utf-8"))
          else:
            rows = query("select count(record_teamrace.Name) from (record_teamrace join record_race on record_teamrace.Map = record_race.Map and record_teamrace.Name = record_race.Name and record_teamrace.Time = record_race.Time) where record_teamrace.Map = '%s' %s group by ID order by count(record_teamrace.Name) desc limit 1;" % (con.escape_string(originalMapName).decode("utf-8"), mbCountry))
    ...
    if 'map' in qs:
      q = qs['map'][0]
      if 'country' in qs:
        newPath = '/maps/%s/%s' % (qs['country'][0].lower(), slugify2(q))
      else:
        newPath = '/maps/%s' % slugify2(q)
      start_response('301 Moved Permanently', [('Location', newPath)])
      return ''

    if 'mapper' in qs:
      q = qs['mapper'][0]
      newPath = '/mappers/%s' % slugify2(q)
      start_response('301 Moved Permanently', [('Location', newPath)])
      return ''
    ...
    if 'query' in qs:
      ...
      FROM (
        ...
        ORDER BY
          ...
          Map
        LIMIT 10
      ) as l;""" % (con.escape_string('%'.join(q)).decode("utf-8"), con.escape_string('%'.join(q)).decode("utf-8"), con.escape_string(q).decode("utf-8"), con.escape_string(q).decode("utf-8"), con.escape_string('%'.join(q)).decode("utf-8"), con.escape_string(q).decode("utf-8"), con.escape_string(q).decode("utf-8")))
      if len(rows) > 0:
        for row in rows:
    ...
    if 'qmapper' in qs:
      ...
      FROM (
        ...
        ORDER BY
          ...
          Mapper
        LIMIT 10
      ) as l;""" % (con.escape_string('%'.join(q)).decode("utf-8"), con.escape_string(q).decode("utf-8"), con.escape_string(q).decode("utf-8")))
      if len(rows) > 0:
        for row in rows:
  ...
  originalMapName = deslugify3(mapName)
  ...
  print('<script src="/mappersearch.js" type="text/javascript"></script>', file=out)
  ...
  mbReleased = ""
  try:
    rows = query("select DATE_FORMAT(Timestamp, '%%Y-%%m-%%d') from record_maps where Map = '%s';" % con.escape_string(originalMapName).decode("utf-8"))
  ...
  if type == "Solo" or type == "Race" or type == "Dummy":
    print(u'<div class="block2 info"><p>%sDifficulty: %s, Points: %d<br/><a href="/mappreview/?map=%s"><img class="screenshot" alt="Screenshot" src="/ranks/maps/%s.png" width="360" height="225" /></a>%s<br/><span title="%s">%d tee%s finished%s</span></p></div>' % (mbReleased, escape(renderStars(stars)), globalPoints(type, stars), quote_plus(originalMapName), escape(normalizedMapName), mbMapInfo, finishTimes, countFinishes, mbS2, escape(avgTime)), file=out)
    watch = watchLinks(originalMapName)
    print(printExactSoloRecords("Records", "records", ranks, not country, watch.get((originalMapName, 'solo'))), file=out)
  else:
    rows = query("select count(distinct ID) from record_teamrace left join record_race on record_teamrace.Map = record_race.Map and record_teamrace.Name = record_race.Name and record_teamrace.Time = record_race.Time where record_teamrace.Map = '%s' %s" % (con.escape_string(originalMapName).decode("utf-8"), mbCountry))
    ...
    watch = watchLinks(originalMapName)
    print(printTeamRecords("Team Records", "teamrecords", teamRanks, not country, watch.get((originalMapName, 'team'))), file=out)
    print(printSoloRecords("Records", "records", ranks, not country, watch.get((originalMapName, 'solo'))), file=out)

  print(printFinishes("Most Finishes", "finishes", finishes), file=out)

  print('<br>', file=out)
  if country:
      print('<a href="/maps/?json=%s&country=%s"><img width="36" src="/json.svg"/></a> Ranks and information for this map on %s server are also available in JSON format.' % (quote_plus(originalMapName), quote_plus(country), country), file=out)
  else:
      print('<a href="/maps/?json=%s"><img width="36" src="/json.svg"/></a> Ranks and information for this map are also available in JSON format.' % quote_plus(originalMapName), file=out)
  print('<br>', file=out)
  print('</div>', file=out)
  print("""  </section>
</article>
