...
build_windows ()
{
  ...
  mkdir $DIR
  cd $DIR
  rustup run 1.89.0 cmake -DVERSION=$VERSION -DCMAKE_BUILD_TYPE=RelWithDebInfo -DDISCORD=ON -DWEBSOCKETS=OFF -DPREFER_BUNDLED_LIBS=ON -DCMAKE_TOOLCHAIN_FILE=cmake/toolchains/mingw$PLATFORM.toolchain -DCMAKE_DISABLE_FIND_PACKAGE_GTest=ON -DEXCEPTION_HANDLING=ON $(echo $BUILDOPTS) ../ddnet-source
...
cp $BUILDDIR/ddnet-source/ddnet-libs/sdl/linux/lib64/libSDL3.so.0 $BUILDDIR/ddnet-source/ddnet-libs/discord/linux/lib64/discord_game_sdk.so ddnet
...
cp $BUILDDIR/ddnet-source/ddnet-libs/sdl/linux/lib32/libSDL3.so.0 ddnet
